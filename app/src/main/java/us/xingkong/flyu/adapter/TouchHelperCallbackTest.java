 ...
 * @更新日志:
 */
public class TouchHelperCallbackTest extends ItemTouchHelper.Callback {

    private PhotosAdapterTest mAdapter;
    private Paint mPaint;    //虚线画笔

    public TouchHelperCallbackTest(PhotosAdapterTest adapter) {
        mAdapter = adapter;
        mPaint = new Paint();
    ...
    public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        if (viewHolder.getLayoutPosition() == mAdapter.getItemCount() - 1
                || viewHolder.getLayoutPosition() == 0) {
            return 0;
        }
    ...
    public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, RecyclerView.ViewHolder target) {
        ...
        Log.e("from&to", "from:" + fromPosition + " to:" + toPosition);
        Log.e("list", mAdapter.getCount() + "");
        if (toPosition == mAdapter.getItemCount() - 1
                || toPosition == 0) {
            return false;
        }
        if (viewHolder instanceof PhotosAdapterTest.ItemHolder) {
    ...
    public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int actionState) {
        ...
        if (actionState == ACTION_STATE_DRAG) {
            if (viewHolder instanceof PhotosAdapterTest.ItemHolder) {
                PhotosAdapterTest.ItemHolder holder = (PhotosAdapterTest.ItemHolder) viewHolder;
